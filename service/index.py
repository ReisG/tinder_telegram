...
async def getplan_command(message : types.Message):
    await message.answer("Ваш план потребления/производства на завтра:")
    with open("grap.jpeg", "rb") as ph:
        await message.answer_photo(types.InputFile(ph))
    keyboard = types.InlineKeyboardMarkup()
    keyboard.insert(types.InlineKeyboardButton(text="отказаться на завтра"))
    await message.answer("За завтра требуется выдать 5 кВт*ч энергии и потребить не более 10 кВт*ч", reply_markup=keyboard)